...
describe('POST /recipes/:id/image', () => {
  describe('when user is not logged in', () => {
    ...
    before(async  () => {
      ...
      response = await chai.request(app).put(`/recipes/${recipeId}/image`).send({});
    });
  ...
  describe('when the user is logged in and the recipe belongs to the user', () => {
    ...
    it('this object has the properties, id, name , ingredients, preparation and userId', function () {
      expect(response.body).to.be.a.property('_id');
      expect(response.body).to.be.a.property('userId');
      expect(response.body).to.be.a.property('image');
      expect(response.body).to.be.a.property('name').equal(validRecipe.name);
      expect(response.body).to.be.a.property('ingredients').equal(validRecipe.ingredients);
      expect(response.body).to.be.a.property('preparation').equal(validRecipe.preparation);
    });
  });
